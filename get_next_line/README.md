...
1. Static buffer
	...
	- In the bonus I made an array of buffers, one for each fd

2. Validation at the start
	- First I check if the fd is valid and if BUFFER_SIZE makes sense
	- I also test if I can actually read from the file
	- If something's wrong, I clean the buffer and return NULL
...
About OPEN_MAX being 64:
In the bonus I have buffer[OPEN_MAX][BUFFER_SIZE + 1]. If OPEN_MAX was 1024 and someone used a huge BUFFER_SIZE, it would try to allocate gigabytes on the stack and crash. So I limited it to 64 which is plenty for most cases.
